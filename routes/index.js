        ...
        Q.all([queryWidgetDatasets(), queryWeekSummaryDataset(), queryYearSummaryDataset(), queryColumnTemperatureDataset(), queryWeekMeasurementsTable(), queryYearHeatmapDataset()]).then(function(results) {
            ...
            queryWeekSummaryDataset.forEach(function(a) {
                ...
                var dayforecast = {measurements:{humidity:{}, pressure:{}, temperature:{}, uv:{}, lx:{}}};
                    ...
                    dayforecast.measurements.uv.avg = a.avgUV;
                    dayforecast.measurements.uv.avgLevel = a.avgUVLvl;
                    dayforecast.measurements.uv.maxLevel = a.maxUVLvl;
            ...
            jsonfile.writeFile(s, response.sparklines, function(err) {});
...
router.get('/', function(req, res, next) {
    var r = '/home/aries/Meteo/cache/request.json';
    fs.appendFile(r, JSON.stringify(req.url.port)+"\n", function(err) {
            if(err) {
    ...
    if(fs.existsSync(file) && parseInt((new Date).getTime()/1000) < parseInt(fs.statSync(file).mtime.getTime()/1000 + 60)) {
        ...
        var response = jsonfile.readFileSync(file);
            response.title = 'Метеостанция Москвы';
        res.render('home', response);
        console.log("IP: "+req.connection.remoteAddress);
        var ipfile = '/home/aries/Meteo/cache/ips.log';
        
        fs.appendFile(ipfile, req.connection.remoteAddress+"\n", function(err) {
            if(err) {
                return console.log(err);
            }
